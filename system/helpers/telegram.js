...
const bot = new Telegraf(global.telegram.token);
/*!-======[ Ends Of Module Imports ]======-!*/
  ...
  for (const file of files) {
    if (file.endsWith('.js')) {
      ...
      try {
        ...
        console.error(`Failed to load feature ${file}:`, error);
      }
    }
  }
...
async function initializeBot(token) {    
    const availableCommands = await loadAllFeatures(bot);    

    try {
      await bot.launch();
      console.log(`Bot with token ${token} has started!`);
    } catch (error) {
      //gabakal error kalo token gada 
    }
...
initializeBot(global.telegram.token);
